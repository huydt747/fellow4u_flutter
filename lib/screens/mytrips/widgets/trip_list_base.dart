...
class _TripListBaseState extends State<TripListBase> {
  ...
  Future<void> _fetchTrips() async {
    ...
    try {
      final trips = await TripService.getTrips(userId, status: widget.tripType.toLowerCase());
      if (mounted) {
        setState(() {
          _trips = trips;
          _isLoading = false;
  ...
  void _openChat(Map<String, dynamic> guide) async {
    ...
    try {
      final conversationId = await ChatService.createConversation(currentUserId, guide['id']);
      if (mounted) {
        Navigator.push(
          context,
          MaterialPageRoute(
  ...
  Widget build(BuildContext context) {
    ...
    if (_trips.isEmpty) {
      return Center(child: Text("No ${widget.tripType.toLowerCase()} trips found"));
    }
      ...
      child: ListView.builder(
        ...
        itemBuilder: (context, index) {
          ...
          return TripCard(
            ...
            time: '8:00 - 10:00', // Placeholder or use dynamic time if available
            person: guide != null ? (guide['fullName'] ?? guide['username']) : 'No Guide',
